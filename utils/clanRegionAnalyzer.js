...
function analyzeTextRegion(clanName, clanTag) {
  ...
  if (hasKoreanCharacters(combinedText)) {
    return { region: 'KR', confidence: 0.9, reason: 'Korean characters in name/tag' };
  }
  
  // 중국어 검사
  if (hasChineseCharacters(combinedText)) {
    return { region: 'CN', confidence: 0.8, reason: 'Chinese characters in name/tag' };
  }
  
  // 일본어 검사
  if (hasJapaneseCharacters(combinedText)) {
    return { region: 'JP', confidence: 0.8, reason: 'Japanese characters in name/tag' };
  }
  
  // 러시아어 검사
  if (hasRussianCharacters(combinedText)) {
    return { region: 'RU', confidence: 0.8, reason: 'Russian characters in name/tag' };
  }
  
  // 아랍어 검사
  if (hasArabicCharacters(combinedText)) {
    return { region: 'ME', confidence: 0.7, reason: 'Arabic characters in name/tag' };
  }
  
  // 한국 관련 키워드 검사 (더 포괄적)
  const koreanKeywords = [
    'korea', 'korean', 'kor', 'kr', 'seoul', 'busan', 'corea',
    'team korea', 'team kr', 'south korea', 'sk', 
    'ubd', 'kim', 'park', 'lee', 'choi', 'jung', // 한국 성씨
    'han', 'kang', 'moon', 'shin', 'yoon', 'lim'
  ];
  ...
  if (clanName) {
    ...
    if (/^[A-Z]{2,4}$/.test(clanName)) {
      return { region: 'KR', confidence: 0.5, reason: 'Korean-style short acronym' };
    }
  ...
  return { region: 'UNKNOWN', confidence: 0.1, reason: 'No regional indicators found' };
}

// 멤버 닉네임 기반 지역 분석
function analyzeMemberNicknames(members) {
  if (!members || members.length === 0) {
    return { region: 'UNKNOWN', confidence: 0, reason: 'No members to analyze' };
  }
  ...
  members.forEach(member => {
    ...
    if (hasKoreanCharacters(nickname)) {
      ...
      // 한국식 패턴 추가 검사
      const isKoreanStyle = koreanPatterns.some(pattern => pattern.test(nickname));
      if (isKoreanStyle) {
        koreanCount++;
      } else {
        englishLikeCount++;
  ...
  console.log(`       닉네임 분석: 한국=${koreanCount}(${Math.round(koreanRatio*100)}%), 영어=${englishLikeCount}(${Math.round(englishRatio*100)}%), 기타=${totalMembers-koreanCount-englishLikeCount}`);
  ...
  if (koreanRatio >= 0.3) {
    return { 
      ...
      stats: { koreanCount, totalMembers, patterns: 'Korean patterns detected' }
    };
  }
  ...
  return { 
    ...
    stats: { koreanCount, chineseCount, japaneseCount, englishLikeCount, totalMembers }
  };
}
...
function analyzeShardDistribution(members) {
  ...
  members.forEach(member => {
    if (member.pubgShardId) {
      shardCounts[member.pubgShardId] = (shardCounts[member.pubgShardId] || 0) + 1;
      totalWithShard++;
    }
  });
  ...
  // 가장 많은 shard 찾기
  const primaryShard = Object.entries(shardCounts)
    .sort(([,a], [,b]) => b - a)[0]?.[0];
...
function analyzeClanRegion(clanData, members = []) {
  ...
  let shardRegionAnalysis = { region: 'UNKNOWN', confidence: 0, reason: 'No shard data' };
