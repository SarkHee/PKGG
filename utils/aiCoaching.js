...
function identifyWeaknesses(stats) {
  ...
  if (avgDamage < 200) weaknesses.push('데미지 딜링 개선 필요 (시즌 평균 낮음)');
  if (top10Rate < 20) weaknesses.push('상위권 진입률 향상 필요 (시즌 평균 낮음)');
  if (winRate < 5) weaknesses.push('승률 개선 필요 (시즌 평균 낮음)');
  if (avgSurvivalTime < 800) weaknesses.push('생존 시간 연장 필요 (시즌 평균 낮음)');
  if (kd < 1.0) weaknesses.push('K/D 비율 개선 필요 (시즌 평균 낮음)');
  if (headshotRate < 20) weaknesses.push('조준 정확도 향상 필요 (시즌 평균 낮음)');
  if (totalMatches < 20) weaknesses.push('더 많은 게임 경험 필요');

  return weaknesses.length > 0 ? weaknesses : ['전반적인 실력 향상 및 경험 축적'];
}

/**
 * 플레이 스타일별 맞춤형 훈련 계획 생성 (시즌 통계 기반)
 */
export function generateTrainingPlan(playStyle, strengths, weaknesses, playerStats) {
    ...
    AGGRESSIVE: [
      {
        ...
        title: '공격적 포지셔닝의 핵심',
        description: '공격할 때는 항상 퇴로를 확보하세요. 적을 공격하기 전 뒤로 빠질 수 있는 엄폐물이나 경로를 미리 파악해두는 것이 중요합니다.',
        priority: 5
      },
      {
        category: 'AIM',
        title: '프리파이어 활용법',
        description: '적이 나올 것으로 예상되는 지점을 미리 조준하고 대기하세요. 반응속도보다는 예측력이 더 중요합니다.',
        priority: 4
      }
    ],
    PASSIVE: [
      {
        category: 'STRATEGY',
        title: '안전한 킬 타이밍',
        description: '다른 팀들이 교전 중일 때가 킬을 얻기 가장 좋은 기회입니다. 써드파티를 적극 활용하되, 자신도 당하지 않도록 주의하세요.',
        priority: 5
      },
      {
        category: 'POSITIONING',
        title: '정보 수집의 중요성',
        description: '높은 곳에서 주변을 관찰하며 적팀의 위치와 이동 패턴을 파악하세요. 정보는 최고의 무기입니다.',
        priority: 4
      }
    ],
    SNIPER: [
      {
        category: 'AIM',
        title: '거리별 조준점 조정',
        description: '각 스나이퍼 라이플의 탄도를 숙지하고, 거리에 따른 조준점 보정을 연습하세요. 100m당 대략 어느 정도 올려야 하는지 체화시키는 것이 중요합니다.',
        priority: 5
      }
    ],
    SUPPORT: [
      {
        category: 'TEAMWORK',
        title: '효과적인 정보 공유',
        description: '적의 위치를 알릴 때는 방향, 거리, 적의 수를 명확히 전달하세요. "저기 적이 있어"보다는 "동쪽 200미터 건물 2층에 2명"이 훨씬 유용합니다.',
        priority: 5
      }
    ]
  };
