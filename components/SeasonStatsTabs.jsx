...
export default function SeasonStatsTabs({ seasonStatsBySeason }) {
  ...
  if (!seasonStatsBySeason || Object.keys(seasonStatsBySeason).length === 0) {
    return <div className="text-gray-500 dark:text-gray-400">통계 데이터가 없습니다.</div>;
  }
  ...
  const renderModeCard = (group) => {
    ...
    if (availableModes.length === 0) {
      // 플레이 데이터가 없는 경우
      return (
        <div key={group.key} className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-lg overflow-hidden">
          {/* 헤더 */}
    ...
    return (
      <div key={group.key} className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-lg overflow-hidden">
        ...
        <div className={`bg-gradient-to-r ${group.color} text-white p-4`}>
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-bold">{group.title}</h3>
            <div className="text-sm opacity-90">{stats?.rounds ?? stats?.roundsPlayed ?? 0} 게임</div>
          </div>
          
          {/* 서브 모드 표시 */}
          <div className="flex gap-2 mt-2">
            {availableModes.map(mode => (
              <span key={mode} className="px-2 py-1 bg-white/20 rounded-md text-xs font-medium">
        ...
        <div className="p-6">
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div className="text-center">
              ...
              <div className="text-sm text-gray-500 dark:text-gray-400">K/D</div>
            </div>
            <div className="text-center">
            ...
            <div>
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium text-green-600 dark:text-green-400">
                  {stats?.winRate ? `${stats.winRate}%` : stats?.winRatio ? `${(stats.winRatio * 100).toFixed(1)}%` : '0.0%'}
                </span>
              </div>
            </div>
            <div>
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium text-blue-600 dark:text-blue-400">
                  {stats?.top10Rate ? `${stats.top10Rate}%` : stats?.top10Ratio ? `${(stats.top10Ratio * 100).toFixed(1)}%` : '0.0%'}
                </span>
              </div>
            </div>
          </div>
            ...
            <div>
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium">
                  {stats?.longestKill ? `${stats.longestKill}m` : stats?.maxDistanceKill ? `${stats.maxDistanceKill}m` : '0.0m'}
                </span>
              </div>
            </div>
            <div>
