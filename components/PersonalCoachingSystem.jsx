...
function analyzePlayerBehavior(playerStats, matches) {
  ...
  if (
    ...
    if (totalMatches > 0) {
      ...
      const totalKills = recentMatches.reduce(
      ...
      const totalSurvivalTime = recentMatches.reduce(
        (sum, match) => sum + (match.survivalTime || match.surviveTime || 0),
        0
      );
      const wins = recentMatches.filter(
        (match) => (match.placement || match.winPlace || 100) === 1
      ).length;
  ...
  console.log('🎯 최종 사용할 통계:', {
    avgDamage,
    avgKills,
    playerKdr,
    winRatio: calculatedStats.winRatio,
  });

  // 교전 타이밍 분석 (실제 경기 데이터 기반)
  if (matches && matches.length > 0) {
    const validMatches = matches.filter(
      (match) => match.survivalTime || match.surviveTime
    );
    const totalSurvivalTime = validMatches.reduce((sum, match) => {
      return sum + (match.survivalTime || match.surviveTime || 0);
    }, 0);

    const avgSurviveTime =
      validMatches.length > 0 ? totalSurvivalTime / validMatches.length : 0;
    const earlyDeaths = validMatches.filter(
      (match) => (match.survivalTime || match.surviveTime || 0) < 300
    ).length; // 5분 미만
    const earlyDeathRate =
      validMatches.length > 0 ? (earlyDeaths / validMatches.length) * 100 : 0;
    ...
    deathAnalysis = [
      {
        ...
        rate:
          totalMatches > 0
            ? Math.round((hotDropDeaths / totalMatches) * 100)
            : 0,
        count: hotDropDeaths,
        icon: '🔥',
      },
      {
        ...
        rate:
          totalMatches > 0
            ? Math.round((midGameDeaths / totalMatches) * 100)
            : 0,
        count: midGameDeaths,
        icon: '⚔️',
      },
      {
        ...
        rate:
          totalMatches > 0
            ? Math.round((lateGameDeaths / totalMatches) * 100)
            : 0,
      ...
      },
      {
        ...
        rate:
          totalMatches > 0
            ? Math.round((lowPlacementDeaths / totalMatches) * 100)
            : 0,
  ...
  const weaponTypes = [
    {
      ...
      recommendation:
        avgDamage < 150
          ? '소총 조준 연습 필요 (현재: ' + Math.round(avgDamage) + ' 데미지)'
          : avgDamage > 250
            ? '소총 활용 우수 (현재: ' + Math.round(avgDamage) + ' 데미지)'
            : '소총 실력 보통 수준 (현재: ' +
    ...
    },
    {
      ...
      recommendation:
        avgKills < 1
          ? '저격총 연습 권장 (현재 평균 킬: ' + avgKills.toFixed(1) + ')'
          : avgKills > 2.5
            ? '저격 실력 우수 (현재 평균 킬: ' + avgKills.toFixed(1) + ')'
    ...
    },
    {
      type: 'CQC (근접전)',
      performance:
        playerKdr > 1.5 ? 'good' : playerKdr > 0.8 ? 'average' : 'poor',
      recommendation:
        playerKdr < 0.8
          ? '근접 교전 승률 향상 필요 (K/D: ' + playerKdr.toFixed(1) + ')'
          : playerKdr > 1.3
            ? '근접 교전 능력 우수 (K/D: ' + playerKdr.toFixed(1) + ')'
  ...
  if (analysis.combatTiming.earlyDeathRate > 40) {
    recommendations.push({
      ...
      priority: 'high',
      icon: '⏰',
      data: `평균 생존시간: ${Math.round(analysis.combatTiming.avgSurviveTime / 60)}분`,
    });
    weaknesses.push(
      '초반 교전 생존율 낮음 (' +
        Math.round(analysis.combatTiming.earlyDeathRate) +
        '%)'
    );
  } else if (analysis.combatTiming.earlyDeathRate < 20) {
    strengths.push(
      '초반 생존 능력 우수 (' +
        Math.round(100 - analysis.combatTiming.earlyDeathRate) +
        '% 생존)'
    );
  }
              ...
              {analysis.strengths.length > 0 ? (
                <ul className="space-y-2">
                  {analysis.strengths.map((strength, idx) => (
                    <li
                      key={idx}
                      className="text-green-800 dark:text-green-200 text-sm flex items-center gap-2"
                    >
              ...
              {analysis.weaknesses.length > 0 ? (
                <ul className="space-y-2">
                  {analysis.weaknesses.map((weakness, idx) => (
                    <li
                      key={idx}
                      className="text-red-800 dark:text-red-200 text-sm flex items-center gap-2"
                    >
              ...
              {analysis.recommendations.map((rec, idx) => (
                <div
                  ...
                  className={`p-4 rounded-lg border shadow-sm ${
                    rec.priority === 'high'
                      ? 'bg-gradient-to-r from-orange-50 to-red-100 dark:from-orange-900/40 dark:to-red-800/40 border-orange-200 dark:border-orange-600'
                      : rec.priority === 'medium'
                        ? 'bg-gradient-to-r from-yellow-50 to-amber-100 dark:from-yellow-900/40 dark:to-amber-800/40 border-yellow-200 dark:border-yellow-600'
            ...
            <div className="grid md:grid-cols-3 gap-4">
              <div className="text-center">
                <div className="text-2xl font-bold text-cyan-700 dark:text-cyan-300">
                  {Math.floor(analysis.combatTiming.avgSurviveTime / 60)}분{' '}
                  {analysis.combatTiming.avgSurviveTime % 60}초
                </div>
              ...
              <div className="text-center">
                <div className="text-2xl font-bold text-green-700 dark:text-green-300">
                  {Math.floor(
                    analysis.combatTiming.recommendedEngagementTime / 60
                  )}
                  분+
                </div>
          ...
          <div className="bg-gradient-to-br from-red-50 to-pink-100 dark:from-red-900/30 dark:to-pink-800/30 border border-red-200 dark:border-red-600 rounded-lg p-4 shadow-sm">
            ...
            <div className="space-y-3">
              {analysis.deathCauses.map((cause, idx) => (
                <div
                  key={idx}
                  className="flex items-center justify-between p-3 bg-white/60 dark:bg-red-800/20 rounded-lg border border-red-100 dark:border-red-700/50"
                >
          ...
          <div className="bg-gradient-to-br from-indigo-50 to-purple-100 dark:from-indigo-900/30 dark:to-purple-800/30 border border-indigo-200 dark:border-indigo-600 rounded-lg p-4 shadow-sm">
            ...
            <div className="space-y-3">
              {analysis.weaponPerformance.map((weapon, idx) => (
                <div
                  key={idx}
                  className="flex items-center justify-between p-3 bg-white/60 dark:bg-indigo-800/20 rounded-lg border border-indigo-100 dark:border-indigo-700/50"
                >
                  <div className="flex-1">
        ...
        <div className="space-y-6">
          <div className="bg-gradient-to-br from-emerald-50 to-teal-100 dark:from-emerald-900/30 dark:to-teal-800/30 border border-emerald-200 dark:border-emerald-600 rounded-lg p-6 shadow-sm">
            ...
            <div className="space-y-4">
              ...
              <div className="bg-white/60 dark:bg-emerald-800/20 rounded-lg p-4 border border-emerald-100 dark:border-emerald-700/50">
                <h4 className="text-amber-700 dark:text-amber-300 font-semibold mb-3">
                  2단계: 전술적 사고
                </h4>
                <ul className="space-y-2 text-sm text-emerald-800 dark:text-emerald-200">
                  <li className="flex items-center gap-2">
              ...
              <div className="bg-white/60 dark:bg-emerald-800/20 rounded-lg p-4 border border-emerald-100 dark:border-emerald-700/50">
                <h4 className="text-purple-700 dark:text-purple-300 font-semibold mb-3">
                  3단계: 고급 전술
                </h4>
                <ul className="space-y-2 text-sm text-emerald-800 dark:text-emerald-200">
                  <li className="flex items-center gap-2">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <div className="bg-blue-100 dark:bg-blue-800/30 border border-blue-200 dark:border-blue-600/50 rounded-lg p-3">
                ...
                <p className="text-sm text-blue-700 dark:text-blue-300">
                  평균 생존시간{' '}
                  {Math.floor(analysis.combatTiming.avgSurviveTime) + 120}초
                  달성하기
                </p>
              </div>
