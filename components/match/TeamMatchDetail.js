...
const TeamMatchDetail = ({ teammatesDetail, nickname, server }) => {
  ...
  const currentPlayer = teammatesDetail.find(
  ...
  // 우리 팀의 총 킬, 총 데미지 계산
  const totalTeamKills = playersInMyTeam.reduce(
    (sum, p) => sum + (p.kills || 0),
    0
  );
  const totalTeamDamage = playersInMyTeam.reduce(
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.teamSummary}>
        <p>
          <Tooltip content="우리 팀의 최종 순위">최종 순위</Tooltip>:{' '}
          <strong style={styles.rankColor}>{myTeamRank}위</strong> /
          <Tooltip content="우리 팀의 총 인원">총 인원</Tooltip>:{' '}
          {myTeamTotalPlayers}명
        </p>
        <p>
          <Tooltip content="우리 팀의 총 킬">총 킬</Tooltip>:{' '}
          <strong style={styles.statHighlight}>{totalTeamKills}</strong> /
          <Tooltip content="우리 팀의 총 데미지">총 데미지</Tooltip>:{' '}
          <strong style={styles.statHighlight}>
            {totalTeamDamage.toFixed(0)}
          </strong>
        </p>
      </div>
      <div style={styles.tableContainer}>
            ...
            {playersInMyTeam.map((player, idx) => {
              ...
              return (
                <tr
                  key={idx}
                  style={
                    player.name.toLowerCase() === nickname.toLowerCase()
                      ? styles.myRow
                      : styles.tr
                  }
                >
                  <td style={styles.td}>
                    <Link
                      href={`/player/${server}/${player.name}`}
                      passHref
                      legacyBehavior
                    >
                      <a style={styles.playerLink}>
                  ...
                  <td style={styles.td}>{player.dbnos || 0}</td>
                  <td style={styles.td}>
                    {survivalMinutes}분 {survivalSeconds}초
                  </td>
                  <td style={styles.td}>{player.opGrade || 'N/A'}</td>
                </tr>
              );
