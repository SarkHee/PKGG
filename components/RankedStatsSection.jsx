...
export default function RankedStatsSection({ rankedSummary, rankedStats, dataSource }) {
  ...
  return (
    <section>
      <h2 className="text-2xl font-bold mb-4 text-gray-800 dark:text-gray-200">경쟁전 통계</h2>
      {hasRanked ? (
        <>
        ...
        <div className="mb-3 p-6 bg-gradient-to-r from-orange-50 to-red-50 border-2 border-orange-200 text-orange-900 rounded-xl shadow-sm">
          <div className="bg-white/60 backdrop-blur-sm rounded-lg p-4 text-center">
            ...
            <div className="text-sm font-medium mb-3">
              <strong>PUBG 공식 API 제한사항:</strong> 경쟁전 데이터는 현재 제한적으로만 제공됩니다
            </div>
            <div className="bg-orange-100 rounded-lg p-3 mb-3">
              <div className="text-xs text-orange-700 space-y-1">
                <div>• <strong>현재 시즌</strong>: 경쟁전 API 데이터 제한</div>
                <div>• <strong>대안</strong>: OP.GG 등은 별도 데이터 소스 사용</div>
                <div>• <strong>참고</strong>: 일반 매치 데이터로 실력 분석 가능</div>
              </div>
            </div>
            <div className="text-xs text-orange-600">
              💡 <strong>대신 확인해보세요:</strong> 시즌 통계, MMR 추이, 딜량 분석
            </div>
          </div>
        </div>
      )}
