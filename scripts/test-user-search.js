...
async function testUserSearch(nickname) {
  try {
    ...
    if (response.ok) {
      ...
      console.log(
        `✅ 플레이어 발견: ${data.player?.attributes?.name || '정보 없음'}`
      );
      console.log(
        `🏢 클랜 정보: ${data.clan?.attributes?.clanName || '클랜 없음'}`
      );
    } else {
    ...
    if (afterCount > beforeCount) {
      console.log(
        `🎉 성공! ${afterCount - beforeCount}명의 새 유저가 DB에 저장됨`
      );
      ...
      newUsers.forEach((user) => {
        console.log(
          `   📋 ${user.nickname} → ${user.clan?.name || '독립 멤버'} (${user.clan?.pubgClanTag || 'N/A'})`
        );
      });
    } else {
