...
async function testApiCall(url, description) {
  ...
  try {
    ...
    if (response.data?.data) {
      console.log(
        `   📊 데이터 개수: ${Array.isArray(response.data.data) ? response.data.data.length : '1개 객체'}`
      );
    }
    return { success: true, data: response.data };
  } catch (error) {
    console.log(
      `   ❌ 실패! Error: ${error.response?.status || error.message}`
    );
    if (error.response?.data) {
      console.log(
        `   📝 응답: ${JSON.stringify(error.response.data, null, 2)}`
      );
    }
