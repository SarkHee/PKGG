...
async function findPlayersFromMatches(playerName, shard = 'steam') {
  ...
  for (const matchRef of matchIds) {
    ...
    if (matchResult.success && matchResult.data?.included) {
      // 매치에 참여한 모든 플레이어 찾기
      const participants = matchResult.data.included.filter(
...
async function discoverViaExistingPlayers() {
  ...
  try {
    ...
    // 각 기존 멤버에 대해
    for (const member of existingMembers.slice(0, 3)) {
      // 처음 3명만 테스트
      console.log(`🎯 ${member.nickname} 분석 중...`);

      for (const shard of shards) {
        // 1. 해당 플레이어의 매치에서 다른 플레이어들 찾기
        const foundPlayers = await findPlayersFromMatches(
          member.nickname,
          shard
        );

        // 2. 찾은 플레이어들 중 일부의 클랜 확인
        for (const playerName of foundPlayers.slice(0, 5)) {
          // 처음 5명만
          ...
          if (result) {
            ...
            if (!discoveredClans.has(clanKey)) {
              ...
              console.log(`    🆕 새 클랜 발견!`);
              console.log(
                `       클랜명: ${result.clan.attributes.name} (${result.clan.attributes.tag})`
              );
              console.log(
                `       레벨: ${result.clan.attributes.level}, 멤버: ${result.clan.attributes.memberCount}명`
              );
              ...
              if (existingClan) {
                ...
                // 간단한 지역 분석
                const tempMembers = [
                  { nickname: result.player.attributes.name },
                ];
                const regionAnalysis = analyzeClanRegion(
                  result.clan.attributes,
                  tempMembers
                );
    ...
    if (discoveredClans.size > 0) {
      ...
      for (const [key, result] of discoveredClans) {
        console.log(
          `   - ${result.clan.attributes.name} (${result.clan.attributes.tag}) - ${result.shard}`
        );
      }
    }
  } catch (error) {
