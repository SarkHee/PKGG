...
async function testApiEndpoint(url, description) {
  ...
  try {
    ...
    if (response.data.data) {
      if (Array.isArray(response.data.data)) {
        ...
        if (response.data.data.length > 0) {
          console.log(`   - 첫 번째 항목 keys: ${Object.keys(response.data.data[0]).join(', ')}`);
          if (response.data.data[0].attributes) {
            console.log(`   - 첫 번째 항목 attributes: ${Object.keys(response.data.data[0].attributes).join(', ')}`);
          }
        }
      } else {
        console.log(`   - data: 객체`);
        ...
        if (response.data.data.attributes) {
          console.log(`   - attributes: ${Object.keys(response.data.data.attributes).join(', ')}`);
        }
        if (response.data.data.relationships) {
          console.log(`   - relationships: ${Object.keys(response.data.data.relationships).join(', ')}`);
        }
      }
    }
    
    if (response.data.included) {
      console.log(`   - included: 배열 (${response.data.included.length}개 항목)`);
      if (response.data.included.length > 0) {
        const types = [...new Set(response.data.included.map(item => item.type))];
    ...
    if (error.response?.status === 404) {
      console.log(`   - 404: 엔드포인트가 존재하지 않거나 리소스를 찾을 수 없음`);
...
async function testClanEndpoints() {
  ...
  for (const test of results) {
    if (test.result.success) {
      ...
      // 멤버 정보가 포함되어 있는지 확인
      if (data.included) {
        const members = data.included.filter(item => item.type === 'member' || item.type === 'player');
        if (members.length > 0) {
          console.log(`\n📍 ${test.description}에서 멤버 정보 발견:`);
          console.log(`   - 멤버 수: ${members.length}명`);
          console.log(`   - 타입: ${members[0].type}`);
          if (members[0].attributes) {
            console.log(`   - 멤버 속성: ${Object.keys(members[0].attributes).join(', ')}`);
          }
          
          // 처음 3명의 멤버 정보 표시
          console.log(`\n   처음 3명의 멤버:`);
          members.slice(0, 3).forEach((member, index) => {
            const name = member.attributes?.name || member.attributes?.nickname || member.id;
