...
const getRandomNickname = () => {
  const base =
    commonKoreanNicknames[
      Math.floor(Math.random() * commonKoreanNicknames.length)
    ];
  ...
  for (let i = 0; i < attempts; i++) {
    ...
    try {
      ...
      if (response.status === 200 && response.data.player) {
        discovered++;
        console.log(
          `✅ [${i + 1}/${attempts}] 발견: ${nickname} → ${response.data.clan?.attributes?.clanName || '클랜 없음'}`
        );
      } else {
      ...
      if (error.response && error.response.status === 404) {
        ...
        console.error(
          `🔥 [${i + 1}/${attempts}] 오류: ${nickname} - ${error.message}`
        );
      }
    }
