...
async function getTeammatesFromMatch(matchId, targetNickname, shard = 'steam') {
  ...
  if (!targetParticipant) {
    console.log(`      ⚠️  타겟 플레이어 ${targetNickname}을 매치에서 찾을 수 없음`);
    return [];
  }
  ...
  console.log(`      👥 팀원 ${teammates.length}명 발견: ${teammates.join(', ')}`);
...
async function discoverClanMembersViaMatches(clanId) {
  try {
    ...
    for (let i = 0; i < clan.members.length; i++) {
      ...
      console.log(`[${i + 1}/${clan.members.length}] ${member.nickname} 분석 중...`);
      ...
      for (const match of matches) {
        ...
        for (const teammate of teammates) {
          if (existingNicknames.has(teammate) || discoveredPlayers.has(teammate)) {
            continue; // 이미 알고 있는 멤버
          }
          
          console.log(`      🔍 ${teammate} 클랜 소속 확인 중...`);
          const isClanMember = await checkIfClanMember(teammate, clan.pubgClanId);
    ...
    if (discoveredPlayers.size > 0) {
      ...
      console.log(`   node scripts/add-discovered-members.js ${clanId} "${newMembers.join(',')}"`)
    } else {
...
async function main() {
  ...
  if (args.length === 0) {
    console.log(`\n📖 사용법:`);
    console.log(`   node scripts/discover-clan-members-via-matches.js [클랜ID]`);
    console.log(`\n📝 예시:`);
