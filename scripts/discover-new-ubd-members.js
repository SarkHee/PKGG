...
async function discoverNewUbdMembers() {
  ...
  try {
    ...
    const dbPlayerIds = new Set(dbMembers.map(m => m.pubgPlayerId).filter(Boolean));
    console.log(`🗃️  DB에 저장된 UBD 멤버: ${dbMembers.length}명`);
    console.log(`📊 PUBG 플레이어 ID가 있는 멤버: ${dbPlayerIds.size}명\n`);
    
    // 3. 새로운 멤버 찾기
    const newMembers = pubgMembers.filter(member => !dbPlayerIds.has(member.id));
    ...
    for (let i = 0; i < Math.min(newMembers.length, 15); i++) {
      ...
      if (playerDetails) {
        ...
        results.failed.push({ playerId: member.id, reason: '플레이어 정보 없음' });
      }
    }
