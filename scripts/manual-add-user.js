      ...
      try {
        ...
        if (response.data.data.length > 0) {
          ...
          if (playerData.attributes.clanId) {
            try {
              const clanResponse = await axios.get(`https://api.pubg.com/shards/${shard}/clans/${playerData.attributes.clanId}`, {
                headers: {
                  Authorization: API_KEY,
                  Accept: 'application/vnd.api+json',
                },
              ...
              console.log(`🏢 클랜 정보: ${clanData.attributes.clanName} (${clanData.attributes.clanTag})`);
            } catch (clanError) {
