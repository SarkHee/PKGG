...
async function getClanInfo(clanId, shard) {
  try {
    ...
    console.error(
      `클랜 정보 가져오기 실패: ${error.response?.status} ${error.response?.statusText}`
    );
    return null;
  }
}
...
async function getPlayerInfo(nickname, shard) {
  try {
    ...
    console.warn(
      `${nickname} 플레이어 정보 가져오기 실패: ${error.response?.status}`
    );
    return null;
  }
}
...
async function findClanMembersInPubgApi(clanId, shard) {
  ...
  for (let i = 0; i < knownMembers.length; i++) {
    ...
    console.log(
      `[${i + 1}/${knownMembers.length}] ${member.nickname} 검증 중...`
    );
    ...
    try {
      ...
      if (playerInfo && playerInfo.attributes.clanId === clanId) {
        ...
        console.log(
          `  ⚠️  클랜 변경: ${member.nickname} (현재 클랜: ${playerInfo.attributes.clanId || '없음'})`
        );
        invalidMembers.push({
...
async function syncUbdClanById() {
  ...
  try {
    ...
    console.log('📈 결과 요약:');
    console.log(
      `  - PUBG 클랜 총 멤버수: ${clanInfo.attributes.clanMemberCount}명`
    );
    ...
    if (coverage < 100) {
      console.log(`\n💡 권장사항:`);
      console.log(
        `  - ${pubgTotalMembers - totalKnownMembers}명의 멤버가 아직 발견되지 않았습니다`
      );
