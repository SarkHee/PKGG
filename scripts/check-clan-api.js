...
async function checkClanApi() {
  ...
  for (const shard of shards) {
    try {
      ...
      console.log(`      멤버 수: ${response.data.data.attributes.clanMemberCount}명`);
      
      // 멤버 목록 API 테스트
      console.log(`\n👥 ${shard} 샤드에서 멤버 목록 확인:`);
      try {
        const membersResponse = await axios.get(`https://api.pubg.com/shards/${shard}/clans/${clanId}/members`, {
          headers: {
            Authorization: API_KEY,
            Accept: 'application/vnd.api+json',
          },
        ...
        console.log(`   ✅ 멤버 목록 API 성공: ${membersResponse.data.data.length}명`);
        
        // 첫 5명의 멤버 정보 출력
        console.log(`   🔍 첫 5명 멤버 샘플:`);
        for (let i = 0; i < Math.min(5, membersResponse.data.data.length); i++) {
          const member = membersResponse.data.data[i];
          console.log(`      ${i + 1}. ID: ${member.id}`);
          
          // 플레이어 상세 정보도 확인
          try {
            const playerResponse = await axios.get(`https://api.pubg.com/shards/${shard}/players/${member.id}`, {
              headers: {
                Authorization: API_KEY,
                Accept: 'application/vnd.api+json',
              },
            ...
            console.log(`         닉네임: ${playerResponse.data.data.attributes.name}`);
          } catch (playerError) {
            console.log(`         닉네임 조회 실패: ${playerError.response?.status || playerError.message}`);
          }
          
          await new Promise(resolve => setTimeout(resolve, 200));
        }
        
        return { shard, membersCount: membersResponse.data.data.length, members: membersResponse.data.data };
        
      } catch (membersError) {
        console.log(`   ❌ 멤버 목록 API 실패: ${membersError.response?.status || membersError.message}`);
      }
      
    } catch (clanError) {
      console.log(`   ❌ ${shard}: ${clanError.response?.status || clanError.message}`);
    }
      ...
      try {
        ...
        if (response.data.data.length > 0) {
          ...
          if (player.relationships?.clan?.data?.id) {
            console.log(`      클랜 관계 ID: ${player.relationships.clan.data.id}`);
          }
        }
      } catch (error) {
        console.log(`   ❌ ${shard}: ${error.response?.status || error.message}`);
      }
