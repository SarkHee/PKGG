...
async function syncClanMembers() {
  ...
  try {
    ...
    for (let i = 0; i < membersToUpdate.length; i++) {
      ...
      console.log(`[${i + 1}/${membersToUpdate.length}] ${member.nickname} 확인 중...`);
      ...
      try {
        ...
        if (result.found) {
          ...
          // 클랜 정보 수집
          if (player.attributes.clanId && !clanUpdates.has(player.attributes.clanId)) {
          ...
          // 멤버를 클랜에 추가
          if (player.attributes.clanId && clanUpdates.has(player.attributes.clanId)) {
            clanUpdates.get(player.attributes.clanId).members.push(member.nickname);
          }
    ...
    for (const [clanId, clanData] of clanUpdates) {
      try {
        ...
        if (existingClan) {
          ...
          console.log(`  ✅ 기존 클랜 업데이트: ${clanData.clanInfo.attributes.clanName}`);
        } else {
          // 새 클랜 생성은 여기서는 하지 않고 수동으로 처리
          console.log(`  ℹ️  새 클랜 발견 (수동 처리 필요): ${clanData.clanInfo.attributes.clanName} (${clanId})`);
        }
    ...
    if (clanUpdates.size > 0) {
      console.log('\n🏆 클랜별 멤버 현황:');
      for (const [clanId, clanData] of clanUpdates) {
        console.log(`  - ${clanData.clanInfo.attributes.clanName} (${clanData.clanInfo.attributes.clanTag}): ${clanData.members.length}명`);
