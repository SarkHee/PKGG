...
async function testCrawling() {
  ...
  for (const url of PUBG_URLS) {
    try {
      ...
      $('a').each((i, elem) => {
        ...
        if (text && href && text.length > 10 && text.length < 200) {
          newsItems.push({
            ...
            url: href.startsWith('http') ? href : `${new URL(url).origin}${href.startsWith('/') ? href : '/' + href}`
          });
        }
      ...
      if (newsItems.length > 0) {
        ...
        // 데이터베이스에 테스트 데이터 저장
        try {
          for (const item of newsItems.slice(0, 3)) { // 상위 3개만 저장
  ...
  try {
    ...
    savedNews.forEach((news, idx) => {
      ...
      console.log(`     카테고리: ${news.category}, 우선순위: ${news.priority}`);
    });
  } catch (dbError) {
