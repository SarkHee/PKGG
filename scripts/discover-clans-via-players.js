...
async function findClanViaPlayer(playerName, shard = 'steam') {
  ...
  console.log(
    `    🏆 클랜 발견: ${clan.attributes.name} (${clan.attributes.tag})`
  );
  console.log(
    `       레벨: ${clan.attributes.level}, 멤버: ${clan.attributes.memberCount}명`
  );
...
async function discoverClansViaPlayers() {
  ...
  try {
    ...
    for (const playerName of koreanStyleNames.slice(0, 5)) {
      // 처음 5개만
        ...
        if (result) {
          ...
          if (!discoveredClans.has(key)) {
            ...
            if (!existingClan) {
              ...
              console.log(
                `       지역 추정: ${regionAnalysis.region} (${Math.round(regionAnalysis.confidence * 100)}%)`
              );
            } else {
