  ...
  try {
    ...
    for (const [index, member] of members.entries()) {
      if (index > 0 && index % 10 === 0) {
        console.log(
          `⏱️  진행률: ${index}/${members.length} (${Math.round((index / members.length) * 100)}%)`
        );
      }
      ...
      try {
        ...
        const response = await axios.get(
          `http://localhost:3000/api/pubg/player?nickname=${member.nickname}`
        );

        if (response.data.player) {
          updated++;
          console.log(
            `✅ ${member.nickname} (${member.clan.name}) - 업데이트됨`
          );
        }
      } catch (error) {
        errors++;
