// GET /api/og/[nickname]?shard=steam
// /report/[nickname] 공유 시 SNS OG 이미지 생성 (Edge Runtime + @vercel/og)
...
export default async function handler(req) {
  ...
  const nickname = decodeURIComponent(url.pathname.split('/').pop() || '')
  ...
  try {
    const res = await fetch(
      `${base}/api/report/${encodeURIComponent(nickname)}?shard=${shard}`,
  ...
  const kills       = d?.avgRealKills  != null ? d.avgRealKills.toFixed(2)        : (d?.avgKills != null ? Number(d.avgKills).toFixed(2) : '-')
  ...
  const totalGames  = d?.totalGames != null ? String(d.totalGames) : '-'
    ...
    (
      ...
      >
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', color: '#A5A0F0', fontSize: 11, fontWeight: 700, letterSpacing: 3 }}>
            PK.GG  SEASON REPORT
          </div>
            ...
            {season
              ? (
                <div style={{
                  ...
                  color: '#63B3ED', fontSize: 11, fontWeight: 700, padding: '2px 10px', borderRadius: 20,
                }}>{season}</div>
              )
              : null}
          </div>
        ...
        <div style={{ display: 'flex', flex: 1, padding: '18px 24px', gap: 24 }}>
          ...
          <div style={{ width: 250, display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <div style={{
                ...
                color: tierColor, fontSize: 11, fontWeight: 700,
                padding: '2px 10px', borderRadius: 20,
              }}>{`${tierEmoji} ${tierLabel}`}</div>
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', color: 'rgba(255,255,255,0.5)', fontSize: 10 }}>Games</div>
              <div style={{ display: 'flex', color: 'white', fontSize: 18, fontWeight: 800 }}>{totalGames}</div>
            </div>
            ...
            {bm
              ? (
                <div style={{
                  ...
                  <div style={{ display: 'flex', color: '#F59E0B', fontSize: 10, fontWeight: 700 }}>Best Match</div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                    <div style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
                      <div style={{ display: 'flex', color: 'white', fontSize: 20, fontWeight: 900 }}>{String(bm.realKills)}</div>
                      <div style={{ display: 'flex', color: 'rgba(255,255,255,0.4)', fontSize: 10 }}>kills</div>
                      <div style={{ display: 'flex', color: 'rgba(255,255,255,0.6)', fontSize: 11, marginLeft: 4 }}>{`${Number(bm.damage).toLocaleString()} dmg`}</div>
                    </div>
                    <div style={{
                      display: 'flex',
                      color: bm.placement === 1 ? '#F59E0B' : 'rgba(255,255,255,0.5)',
                      fontSize: 11, fontWeight: 700,
                    }}>{bm.placement === 1 ? '#1 Chicken!' : `#${bm.placement}`}</div>
                  </div>
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              {[
                { label: isCorrected ? 'Real Kills' : 'Avg Kills', value: kills,   sub: isCorrected ? 'bot excluded' : '', color: '#A5A0F0' },
                { label: isCorrected ? 'Real Damage' : 'Avg Damage', value: damage, sub: isCorrected ? 'bot excluded' : '', color: '#A5A0F0' },
                { label: 'Win Rate',                                  value: winRate, sub: '',                               color: '#34D399' },
                ...
                <div key={label} style={{
                  ...
                  {sub
                    ? <div style={{ display: 'flex', color, fontSize: 9, marginTop: 2 }}>{sub}</div>
                    : null}
                </div>
              ))}
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              <div style={{ display: 'flex', color: 'rgba(255,255,255,0.4)', fontSize: 10, fontWeight: 700, letterSpacing: 1 }}>
                TOP 3 WEAPONS
              </div>
              {topWeapons.length > 0
                ? topWeapons.map((w, i) => {
                    ...
                    return (
                      <div key={w.weaponId || i} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        <div style={{ display: 'flex', width: 90, color: 'white', fontSize: 11, fontWeight: 700 }}>{w.name}</div>
                        ...
                        <div style={{ display: 'flex', color: BAR_COLORS[i], fontSize: 11, fontWeight: 800, width: 44, justifyContent: 'flex-end' }}>{`${w.kills} kills`}</div>
                      </div>
                    )
                  })
                : (
                  <div style={{ display: 'flex', color: 'rgba(255,255,255,0.25)', fontSize: 11 }}>No match data</div>
                )}
            </div>
          </div>
        </div>
