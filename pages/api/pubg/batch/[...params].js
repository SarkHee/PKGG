...
export default async function handler(req, res) {
  ...
  if (!Array.isArray(params) || params.length < 2) {
    return res.status(400).json({
      error:
        '잘못된 경로입니다. /api/pubg/batch/{type}/{shard}/... 형식을 사용하세요.',
    });
  }
    ...
    switch (type) {
      ...
      case 'stats':
        if (rest.length < 2) {
          return res.status(400).json({
            error:
              '/api/pubg/batch/stats/{shard}/{seasonId}/{gameMode} 형식이 필요합니다.',
          });
        }
      ...
      case 'lifetime':
        if (rest.length < 1) {
          return res.status(400).json({
            error:
              '/api/pubg/batch/lifetime/{shard}/{gameMode} 형식이 필요합니다.',
          });
        }
      ...
      default:
        return res.status(400).json({
          error:
            '지원하지 않는 타입입니다. players, stats, lifetime 중 하나를 사용하세요.',
        });
    }
  } catch (error) {
...
async function handleStatsBatch(req, res, shard, seasonId, gameMode) {
  ...
  try {
    const data = await fetchSeasonStatsBatch(
      shard,
      seasonId,
      gameMode,
      playerIds
