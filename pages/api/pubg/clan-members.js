...
export default async function handler(req, res) {
  ...
  if (!playerNames) {
    return res
      .status(400)
      .json({ error: 'playerNames is required (comma-separated)' });
  }

  const API_KEY =
    'Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJqdGkiOiI3MDNhNDhhMC0wMjI1LTAxM2UtMzAwYi0wNjFhOWQ1YjYxYWYiLCJpc3MiOiJnYW1lbG9ja2VyIiwiaWF0IjoxNzQ1MzgwODM3LCJwdWIiOiJibHVlaG9sZSIsInRpdGxlIToicHViZyIsImFwcCI6InViZCJ9.hs5WCvTM6d0W_y0lsYzpbkREq61PD1p7vbibOGTFK3o';

  try {
    const names = playerNames
      .split(',')
      .map((name) => name.trim())
      .slice(0, 6); // PUBG API 한번에 최대 6명
    const nameParams = names.join(',');

    const response = await axios.get(
      `https://api.pubg.com/shards/${shard}/players?filter[playerNames]=${nameParams}`,
      {
        headers: {
    ...
    console.error(
      'Clan members fetch error:',
      error.response?.data || error.message
    );
    return res
      .status(500)
      .json({ error: 'Failed to fetch clan members information' });
  }
}
