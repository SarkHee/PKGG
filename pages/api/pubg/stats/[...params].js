  ...
  try {
    switch (type) {
      case 'season':
        if (rest.length < 1) {
          return res.status(400).json({ 
            error: 'seasonId가 필요합니다. /api/pubg/stats/season/{shard}/{playerId}/{seasonId}' 
          });
        }
      ...
      case 'ranked':
        if (rest.length < 1) {
          return res.status(400).json({ 
            error: 'seasonId가 필요합니다. /api/pubg/stats/ranked/{shard}/{playerId}/{seasonId}' 
          });
        }
      ...
      case 'mastery':
        if (rest.length < 1) {
          return res.status(400).json({ 
            error: '숙련도 타입이 필요합니다. /api/pubg/stats/mastery/{shard}/{playerId}/{weapon|survival}' 
          });
        }
      ...
      data: {
        player: {
          ...
          name: data.data.attributes.gameModeStats ? 
            Object.keys(data.data.attributes.gameModeStats)[0] : 'Unknown'
        },
        season: {
...
async function handleMasteryStats(req, res, shard, playerId, masteryType) {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`PUBG API ${masteryType} 숙련도 조회 실패: ${response.status}`);
    }
