...
export default async function handler(req, res) {
  ...
  if (!matchIds || !playerName || !shardId) {
    return res.status(400).json({ error: '필수 데이터 누락: matchIds, playerName, shardId' });
  }

  const API_KEY = 'Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJqdGkiOiI3MDNhNDhhMC0wMjI1LTAxM2UtMzAwYi0wNjFhOWQ1YjYxYWYiLCJpc3MiOiJnYW1lbG9ja2VyIiwiaWF0IjoxNzQ1MzgwODM3LCJwdWIiOiJibHVlaG9sZSIsInRpdGxlIjoicHViZyIsImFwcCI6InViZCJ9.hs5WCvTM6d0W_y0lsYzpbkREq61PD1p7vbibOGTFK3o'; // ← 여기에 본인의 PUBG API 키 입력
  ...
  try {
    ...
    return res.status(500).json({ error: '서버 오류: 경기 데이터 처리 중 문제 발생' });
  }
}
