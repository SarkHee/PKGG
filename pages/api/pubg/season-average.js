...
export default async function handler(req, res) {
  ...
  try {
    ...
    const seasonRes = await axios.get(
      `https://api.pubg.com/shards/${shardId}/seasons`,
      {
        headers: {
    ...
    const currentSeason = seasonRes.data.data.find(
    ...
    const statsRes = await axios.get(
      `https://api.pubg.com/shards/${shardId}/players/${accountId}/seasons/${seasonId}`,
      {
        headers: {
          Authorization: API_KEY,
          Accept: 'application/vnd.api+json',
    ...
    const average = matchesPlayed
      ? (totalDamage / matchesPlayed).toFixed(1)
      : '0.0';
    ...
    return res
      .status(500)
      .json({ error: '시즌 평균 대미지를 불러오지 못했습니다.' });
  }
}
