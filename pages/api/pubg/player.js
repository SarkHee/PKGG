        ...
        if (response.data.data.length > 0) {
          ...
          if (player.attributes.clanId) {
            try {
              ...
              const discoveryResult = await saveOrUpdateClanAndMember(player, clanInfo, shard);
              
              // 발견 결과 로깅
              if (discoveryResult.newClan || discoveryResult.newMember) {
                console.log(`🔍 자동 발견 완료 - 플레이어: ${player.attributes.name}`);
                if (discoveryResult.newClan) console.log(`   ✨ 새 클랜: ${clanInfo.attributes.clanName} (${clanInfo.attributes.clanTag})`);
                if (discoveryResult.newMember) console.log(`   ✨ 새 멤버: ${player.attributes.name} → ${clanInfo.attributes.clanName}`);
                console.log(`   📊 현재 DB 상태: ${discoveryResult.totalClans}개 클랜, ${discoveryResult.totalMembers}명 멤버`);
              }
            ...
            console.log(`🔍 클랜 없는 플레이어 발견: ${player.attributes.name}`);
            const discoveryResult = await saveOrUpdateIndependentPlayer(player, shard);
...
async function saveOrUpdateClanAndMember(player, clanInfo, shard) {
  ...
  try {
    ...
    if (existingClan) {
      ...
      newClan = true;
      console.log(`🎉 새 클랜 자동 생성: ${clanInfo.attributes.clanName} (${clanInfo.attributes.clanTag})`);
    }
    ...
    if (existingMember) {
      ...
      newMember = true;
      console.log(`🎉 새 멤버 자동 생성: ${player.attributes.name} → ${clanInfo.attributes.clanName}`);
