...
export default function ClanAnalytics() {
  ...
  // 게이트 표시 조건: 비로그인(user===null) 또는 로그인+PUBG 미연동 확정(myUserData 로딩 완료 후)
  // myUserData가 아직 로딩 중(undefined)이면 깜빡임 방지를 위해 게이트 미노출
  const needsAuthGate = !isAdmin && (user === null || (!!user && myUserData !== undefined && !hasLinkedPubg));
  ...
  return (
    <>
          ...
          <div className="bg-gray-900 border border-gray-700 rounded-2xl shadow-2xl p-8 max-w-sm w-full mx-4 text-center">
            ...
            {user === null ? (
              ...
              <>
                ...
                <p className="text-sm text-gray-400 mb-6 leading-relaxed">
                  PUBG 계정을 연동해야 클랜 분석을 이용할 수 있습니다.
                </p>
                <Link
                  href="/mypage"
      ...
      <div className={`min-h-screen bg-gray-950 px-4 py-8 ${needsAuthGate ? 'blur-sm pointer-events-none select-none' : ''}`}>
        <div className="max-w-6xl mx-auto">
          ...
          {/* 클랜 미가입 안내 배너 — PUBG 연동은 됐지만 클랜 소속이 없는 경우만 (미연동은 위 오버레이가 처리) */}
          {user && hasLinkedPubg && !myClanId && !isSteamAdmin && !isAdmin && (
