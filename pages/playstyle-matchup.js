...
function scoreLabel(score, t) {
  if (score >= 5) return t('pm.score_very_favorable');
  if (score === 4) return t('pm.score_favorable');
  if (score === 3) return t('pm.score_similar');
  if (score === 2) return t('pm.score_unfavorable');
  return t('pm.score_very_unfavorable');
}
  ...
  return (
    <div className="min-h-screen bg-gray-950">
      <Head>
        ...
        <meta name="description" content={t('pm.meta_desc')} />
      </Head>
      <Header />
      ...
      <main className="max-w-screen-xl mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          <h1 className="text-2xl font-black text-white mb-2">{t('pm.heading')}</h1>
          <p className="text-gray-400 text-sm">{t('pm.subheading')}</p>
        </div>
        ...
        {!selected && (
          <div className="text-center py-16">
            ...
            <p className="text-lg font-medium text-gray-500">{t('pm.select_hint1')}</p>
            <p className="text-sm text-gray-600 mt-1">{t('pm.select_hint2')}</p>
          </div>
        )}
          ...
          <div>
            ...
            <div className={`flex items-center gap-4 p-5 rounded-2xl border mb-6 ${selectedType.bg} ${selectedType.border}`}>
              ...
              <div className="text-right flex-shrink-0">
                <div className="text-xs text-gray-500 mb-0.5">{t('pm.avg_score')}</div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              ...
              <div className="bg-emerald-500/5 border border-emerald-500/20 rounded-2xl p-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="font-bold text-emerald-400 text-sm">{t('pm.win_group').replace('{n}', matchups.win.length)}</span>
                </div>
                {matchups.win.length === 0
                  ? <p className="text-xs text-gray-600 py-2">{t('pm.none')}</p>
                  : (
                        ...
                        return (
                          <div key={key} className="flex items-center gap-3 bg-gray-900/60 rounded-xl p-2.5">
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              <div className="text-[10px] text-emerald-400">{scoreLabel(score, t)}</div>
                            </div>
              ...
              <div className="bg-yellow-500/5 border border-yellow-500/20 rounded-2xl p-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="font-bold text-yellow-400 text-sm">{t('pm.neutral_group').replace('{n}', matchups.neutral.length)}</span>
                </div>
                      ...
                      {matchups.neutral.map((key) => {
                        ...
                        return (
                          <div key={key} className="flex items-center gap-3 bg-gray-900/60 rounded-xl p-2.5">
                            ...
                            <div className="min-w-0 flex-1">
                              <div className="text-xs font-semibold text-gray-200 truncate">{tp?.label?.split(' ').slice(1).join(' ')}</div>
                              <div className="text-[10px] text-yellow-400">{t('pm.similar')}</div>
                            </div>
              ...
              <div className="bg-red-500/5 border border-red-500/20 rounded-2xl p-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="font-bold text-red-400 text-sm">{t('pm.lose_group').replace('{n}', matchups.lose.length)}</span>
                </div>
                      ...
                      {matchups.lose.map((key) => {
                        ...
                        return (
                          <div key={key} className="flex items-center gap-3 bg-gray-900/60 rounded-xl p-2.5">
                            ...
                            <div className="min-w-0 flex-1">
                              <div className="text-xs font-semibold text-gray-200 truncate">{tp?.label?.split(' ').slice(1).join(' ')}</div>
                              <div className="text-[10px] text-red-400">{scoreLabel(score, t)}</div>
                            </div>
            ...
            <div className="p-4 bg-gray-900/50 border border-gray-800 rounded-2xl text-xs text-gray-600 mb-6">
              <strong className="text-gray-400">{t('pm.note_label')}</strong> {t('pm.note_desc')}
              {t('pm.legend')}
            </div>
          </div>
        )}
        ...
        >
          {showMatrix ? t('pm.hide_matrix') : t('pm.show_matrix')}
        </button>
        ...
        {showMatrix && (
          <div className="mt-4 overflow-x-auto rounded-2xl border border-gray-800">
            <div className="flex flex-wrap gap-3 p-3 bg-gray-900/60 border-b border-gray-800">
              {[
                {s:5,l:t('pm.score_very_favorable')},
                {s:4,l:t('pm.score_favorable')},
                {s:3,l:t('pm.score_similar')},
                {s:2,l:t('pm.score_unfavorable')},
                {s:1,l:t('pm.score_very_unfavorable')},
              ].map(({s,l}) => (
              ...
              <span className="text-[10px] text-gray-600 ml-2">{t('pm.matrix_hint')}</span>
            </div>
                        ...
                        return (
                          <td key={colKey} className="p-1 text-center">
                            <div
                              ...
                              title={isSelf ? t('pm.same_type') : `${rowType?.label} vs ${TYPES[colKey]?.label}: ${scoreLabel(score, t)}`}
                            >
                              {isSelf ? '–' : score}
                            </div>
                          </td>
